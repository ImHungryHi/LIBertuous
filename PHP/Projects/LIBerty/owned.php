<?php
	 ...
	 */
	 	...
	 	if (isset($_POST['formAction']) && $_POST['formAction'] == 'search')
	 	{
			...
			{
				// Filter that stuff
				$arrMovies = getFilteredOwned((int) $_SESSION['userid'], $txtSearch);
			}
	 	}
	 ...
	 */
		...
		$pageTpl = new Template('./templates/owned.tpl');

		$mainTpl->assign('pageTitle', 'LIBerty - owned');
		$mainTpl->assign('pageMeta', '<link rel="stylesheet" type="text/css" href="./core/styles/owned.css" />');
		...
		{
			// Select ALL movies
			$arrMovies = getOwned((int) $_SESSION['userid']);
		}
