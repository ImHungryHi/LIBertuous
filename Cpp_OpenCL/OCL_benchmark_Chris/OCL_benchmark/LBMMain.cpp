...
void update()
{
	...
	{
		char title[80];
		sprintf_s(title, "Lattice Boltzmann demo    %.1f fps", (1000.0*frames/(te-t0)));
...
void motion(int x, int y)
{
	...
	if (!addVelocity && lb->getMotionStatus())
	{
		lb->setMotionOff();
	}
}
...
void reshape(int w, int h)
{
	winwidth = w;
	winheight = h;
	cout << "Changed window dimensions to " << w << "x" << h << "\n";
}
...
void keyboard(unsigned char key, int x, int y)
{
	if (key == 27)
	{
		cout << "Exiting program in a second\n";
		Sleep(1000);
		exit(0);
	}
	...
	{
		drawVelocities *= -1;
		cout << "Toggled velocity colour\n";
	}
	else if (key == 'p')
	{
		drawParticles *= -1;
		cout << "Toggled particles\n";
	}
}
...
{
	return lb->GetAverageCalculationsPerSecond();
}

void ExecuteLBM()
{
	Platform platform = util::GetPlatform();
	Device device = util::GetDevice(platform);
	Context context = util::CreateContext(device.getInfo<CL_DEVICE_TYPE>());
	Kernel kernel = util::LoadKernelFromFile(context, "LBMKernels.cl", "LBM");
